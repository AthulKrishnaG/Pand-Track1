        ...
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-6 col-md-offset-3 text-center">
              <h2 class="text-theme-colored font-36">My Source Entries</h2>
              <ol class="breadcrumb text-center mt-10 white">
                <li><a href="dashboard.php">Dashboard</a></li>
                <li class="active">My Sources</li>
              </ol>
            </div>
          </div>
        </div>
    ...
    <section class="divider parallax layer-overlay overlay-deep" data-stellar-background-ratio="0.2" data-bg-img="">
      <div class="container">
        <div class="section-content text-center">
          <div class="row">
            <div class="col-md-12">
              <h3 class="mt-0">Your Submitted Sources</h3>

              <div data-example-id="hoverable-table" class="bs-example"> 
                <table class="table table-hover table-bordered">
                  <?php
                    // Fetch all source records for this patient
                    $select = "
                      SELECT 
                        d_source_id,
                        d_source_confirmed_location,
                        d_source_state,
                        d_source_district,
                        d_source_panchayath,
                        d_source_ward_no,
                        d_source_in_date
                      FROM `pantra`.`source`
                      WHERE d_source_patient = '$uID'
                      ORDER BY d_source_in_date DESC
                    ";
                    ...
                    if ($num > 0) {
                      while ($row = mysqli_fetch_assoc($result)) {
                        $i++;
                        $id         = $row['d_source_id'];
                        $location   = htmlspecialchars($row['d_source_confirmed_location']);
                        $state      = htmlspecialchars($row['d_source_state']);
                        $district   = htmlspecialchars($row['d_source_district']);
                        $panchayath = htmlspecialchars($row['d_source_panchayath']);
                        $ward_no    = $row['d_source_ward_no'];
                        $in_date    = $row['d_source_in_date'];
                      ...
                      echo "
                        <tr>
                          <td colspan='7' class='text-center'>No source entries found.</td>
                        </tr>
                      ";
                    }
