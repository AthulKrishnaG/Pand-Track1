...
<div id="wrapper">
        ...
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-6 col-md-offset-3 text-center">
              <h2 class="text-theme-colored font-36">Hotspot</h2>
  ...
  <div class="container">
    <div class="section-content text-center">
      <div class="row">
        <div class="col-md-12">
          <h3 class="mt-0">All Hotspots History</h3>

          <div data-example-id="hoverable-table" class="bs-example"> 
            <table class="table table-hover table-bordered">
              <?php
                // Fetch all hotspot records
                $select = "
                  SELECT 
                    h_id,
                    h_state,
                    ...
                    h_ward_no,
                    h_confirmed_area,
                    DATE_FORMAT(h_in_date, '%d-%m-%Y %h:%i %p') AS submitted_on
                  FROM `pantra`.`hotspot`
                  ORDER BY h_in_date DESC
                ";
                ...
                if ($num > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    $i++;
                    $state          = htmlspecialchars($row['h_state']);
                    $district       = htmlspecialchars($row['h_district']);
                    $taluk          = htmlspecialchars($row['h_taluk']);
                    $block          = htmlspecialchars($row['h_block']);
                    $panchayath     = htmlspecialchars($row['h_panchayath']);
                    $ward_no        = htmlspecialchars($row['h_ward_no']);
                    $confirmed_area = htmlspecialchars($row['h_confirmed_area']);
                  ...
                  echo "
                    <tr>
                      <td colspan='9' class='text-center'>No hotspots found.</td>
                    </tr>
                  ";
                }
