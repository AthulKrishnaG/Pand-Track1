...
<div id="wrapper">
        ...
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-6 col-md-offset-3 text-center">
              <h2 class="text-theme-colored font-36">Medicine Entries</h2>
  ...
  <div class="container">
    <div class="section-content text-center">
      <div class="row">
        <div class="col-md-12">
          <h3 class="mt-0">All Medicines History</h3>

          <div data-example-id="hoverable-table" class="bs-example"> 
            <table class="table table-hover table-bordered">
              <?php
                // Fetch all medicine records (no filtering by patient)
                $select = "
                  SELECT 
                    d_m_id,
                    d_m_name,
                    d_m_dose,
                    DATE_FORMAT(d_m_startingdate, '%d-%m-%Y') AS starting_date,
                    d_m_duration,
                    DATE_FORMAT(d_m_indate, '%d-%m-%Y %h:%i %p') AS submitted_on
                  FROM `pantra`.`medicines`
                  ORDER BY d_m_indate DESC
                ";
                ...
                if ($num > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    $i++;
                    $name         = htmlspecialchars($row['d_m_name']);
                    $dose         = htmlspecialchars($row['d_m_dose']);
                    $start_date   = $row['starting_date'];
                    $duration     = htmlspecialchars($row['d_m_duration']);
