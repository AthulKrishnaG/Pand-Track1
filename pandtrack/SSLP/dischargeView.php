...
<div id="wrapper">
  ...
  <div class="main-content">
        ...
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-6 col-md-offset-3 text-center">
              <h2 class="text-theme-colored font-36">Discharge Entries</h2>
              <ol class="breadcrumb text-center mt-10 white">
                <li><a href="index.php">Home</a></li>
                <li class="active">Discharge Details</li>
              </ol>
            </div>
          </div>
        </div>
    ...
    <!-- Section: Display only this patient's History records -->
    <section class="divider parallax layer-overlay overlay-deep" data-stellar-background-ratio="0.2" data-bg-img="">
  <div class="container">
    <div class="section-content text-center">
      <div class="row">
        <div class="col-md-12">
          <h3 class="mt-0">Your Discharge Records</h3>

          <div data-example-id="hoverable-table" class="bs-example"> 
            <table class="table table-hover table-bordered">
              <?php
                // Fetch discharge records for the logged-in patient by email
                // Assumes $uEmail is set from session.php
                $select = "
                  SELECT 
                    discharge_p_id,
                    discharge_p_name,
                    discharge_p_gender,
                    discharge_p_mobilenum,
                    discharge_p_address,
                    discharge_p_email,
                    discharge_p_type,
                    DATE_FORMAT(discharge_p_in_date, '%d-%m-%Y %h:%i %p') AS submitted_on
                  FROM `pantra`.`discharge`
                  WHERE discharge_p_email = '$uEmail'
                  ORDER BY discharge_p_in_date DESC
                ";
                ...
                if ($num > 0) {
                  while ($row = mysqli_fetch_assoc($result)) {
                    $i++;
                    $name         = htmlspecialchars($row['discharge_p_name']);
                    $gender       = htmlspecialchars($row['discharge_p_gender']);
                    $mobile       = htmlspecialchars($row['discharge_p_mobilenum']);
                    $address      = htmlspecialchars($row['discharge_p_address']);
                    $email        = htmlspecialchars($row['discharge_p_email']);
                    $type         = htmlspecialchars($row['discharge_p_type']);
                  ...
                  echo "
                    <tr>
                      <td colspan='8' class='text-center'>No discharge records found.</td>
                    </tr>
                  ";
                }
