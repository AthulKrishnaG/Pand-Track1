...
VALUES (NULL,'$uID','$d_source_confirmed_location', '$d_source_state', '$d_source_district', '$d_source_panchayath','$d_source_ward_no', '$d_source_in_date','$d_source_up_date')";
	$insertresult=mysqli_query($dbconnection,$insertqry);
	//Check whether the query was successful or not
	if($insertresult) 
	{
		$msg = urlencode("Successfully added source");
...
VALUES (NULL, '$c_patient', '$c_district', '$c_visited_places', '$c_explain_the_activities','$c_explanation_activities_date','$c_explanation_activities_time','$c_in_date','$c_up_date')";
	...
	if($insertresult) 
	{    
		$msg = urlencode("Contact added successfully");
	...
	{
		$msg = urlencode("Failed to add contact");
		header("Location: error.php?msg=$msg");
		exit();
	}	
} 
...
VALUES (NULL, '$c_p_patient','$c_p_person', '$c_p_district', '$c_p_town', '$c_p_place','$c_p_in_date','$c_p_up_date')";
	...
	if($insertresult) 
	{
		$msg = urlencode("Primary contact added successfully");
	...
	{
		$msg = urlencode("Failed to add primary contact");
		header("Location: error.php?msg=$msg");
		exit();
	}	
}
