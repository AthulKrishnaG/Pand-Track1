...
<div id="wrapper">
  ...
  <div class="main-content">
        ...
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-6 col-md-offset-3 text-center">
              <h2 class="text-theme-colored font-36">My Primary Contacts</h2>
              <ol class="breadcrumb text-center mt-10 white">
                <li><a href="index.php">Home</a></li>
                <li class="active">My Primary Contacts</li>
              </ol>
            </div>
          </div>
        </div>
    ...
    <!-- Section: Display only this patient's Primary Contact records -->
    <section class="divider parallax layer-overlay overlay-deep" data-stellar-background-ratio="0.2" data-bg-img="">
      <div class="container">
        <div class="section-content text-center">
          <div class="row">
            <div class="col-md-12">
              <h3 class="mt-0">Your Submitted Primary Contacts</h3>

              <div data-example-id="hoverable-table" class="bs-example"> 
                <table class="table table-hover table-bordered">
                  <?php
                    // Fetch all primary_contact records for this patient
                    $select = "
                      SELECT 
                        c_p_id,
                        c_p_person,
                        ...
                        c_p_place,
                        c_p_in_date
                      FROM `pantra`.`primary_contact`
                      WHERE c_p_patient = '$uID'
                      ORDER BY c_p_in_date DESC
                    ";
                    ...
                    if ($num > 0) {
                      while ($row = mysqli_fetch_assoc($result)) {
                        $i++;
                        $pid      = $row['c_p_id'];
                        $person   = htmlspecialchars($row['c_p_person']);
                        $district = htmlspecialchars($row['c_p_district']);
                        $town     = htmlspecialchars($row['c_p_town']);
                        $place    = htmlspecialchars($row['c_p_place']);
                        $in_date  = $row['c_p_in_date'];
                      ...
                      echo "
                        <tr>
                          <td colspan='6' class='text-center'>No primary contact entries found.</td>
                        </tr>
                      ";
                    }
